...
(function (global, factory) {
    ...
    var _typeof = function (obj) {
        ...
        const _t = ['dialog', 'confirm', 'toast', 'alert', 'notify']
        if (!t) t = _t[0]
        if (_inarray(_t, t) === false) return _error('Oops! type must be equal ' + _t)
        return t
    }, _icon = function (i) {
        const _i = [false, 'success', 'error', 'question', 'warning', 'info']
        if (i) if (_inarray(_i, i) === false) return _error('Oops! icon must be equal ' + _i)
        return i
    }, _evt = function (e) {
        const _evt = ['default', 'dark', 'success', 'error', 'question', 'warning', 'info']
        ...
        const _p = ['center', 'center-top', 'center-bottom', 'bottom-right', 'bottom-left', 'top-left', 'top-right']
        if (p) if (_inarray(_p, p) === false) return _error('Oops! position must be equal' + _p)
        return p
    }, _animate = function (a) {
        const _a = ['popup', 'scaleIn', 'bounce', 'pluse', 'fadeIn', 'rollIn', 'ripple', 'slideIn', 'slide']
        if (!a) a = _a[0]
        if (_inarray(_a, a) === false) return _error('Oops! animate must be equal ' + _a)
        return a
    }, _delay = function (d) {
        const _d = ['number', 'boolean']
        ...
        const h = _addElement(el, [t + '-header'], { label: 'header' })
        ...
        var el = 'div'
        if (t === 'toast' || t === 'notify') el = 'span'
        const b = _addElement(el, [t + '-body', 'txt'], { label: 'body' })
        ...
        const p = _addElement('p', ['paragth', 'txt'], { label: 'paragth' })
        _innerText(p, c.m)
        if (t === 'alert') return p
        _append(b, p)
        return b
        ...
        var el = 'div'
        if (t === 'toast' || t === 'notify') el = 'span'
        const f = _addElement(el, [t + '-footer'], { label: 'footer' })
        const clases = ['btn', 'btn-default', 'btn-modal']
        const attr = {
            id: null,
            type: 'button',
            'aria-hidden': true,
        ...
        const di = _addElement('div', [type], attr)
        di.style.width = w;
        ...
        const c = el.querySelectorAll('[aria="btn-dark"]')
        if (c) c.forEach(el => {
        ...
        if (b === true) {
            if (t === 'toast' || t === 'notify') {
                ...
                if (o) for (let e = 0; e < o.length; e++) {
                    ...
                    const _c = o[e].querySelector('[aria="btn-dark"]')
            ...
            if (t === 'confirm' || t === 'dialog') {
                var o = el
                const _c = o.querySelector('[aria="btn-dark"]')
                o.addEventListener('click', function (e) {
        ...
        const el = _addElement('div', ['overlay'], { id: 'overlay', aria: a.type, animate: a.animate, ...p })
